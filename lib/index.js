...
function emptyUsageStore() {
  // v3: per-model buckets keep cache-hit reads apart from cache-miss input,
  // and accumulate the estimated cost in the peak / idle billing bands.
  return { version: 3, days: {}, months: {}, total: {}, steps: {}, cursors: {} };
}
...
function repairUsageStore(store) {
  ...
  for (const periods of [store.days, store.months]) {
    for (const bucket of Object.values(periods || {})) {
      ...
      const stamped = hasOwnProperty.call(bucket, "costPeak") || hasOwnProperty.call(bucket, "costOff");
      if (stamped) {
