...
function emptyUsageStore() {
  ...
  return { version: 9, days: {}, months: {}, total: {}, steps: {}, cursors: {} };
}
 ...
 * - Kimi（Moonshot）：官方美元价 → 按 USD_TO_CNY 折算（无峰谷，恒价）。K3 缓存命中价
 *   未公布，按官方 K2 系约 6× 折扣规律估算为输入的 ~1/6。
 * - GLM（Z.AI / 智谱）：官方美元价 → 按 USD_TO_CNY 折算（docs.z.ai/guides/overview/pricing，
 *   无峰谷，恒价）。
...
const MODEL_PRICING = [
  ...
  // Kimi（Moonshot）官方美元价折算；K2.7 Code 与 K2.6 同价
  { match: ["k3"], source: "Kimi 官方(≈×7.2)", rates: flatRates(0.5, 3.0, 15.0) },
  { match: ["k2.7", "k2.6"], source: "Kimi 官方(≈×7.2)", rates: flatRates(0.16, 0.95, 4.0) },
...
const PRICING_INFO = {
  ...
  note: "费用按各厂商官方价估算（统一人民币，非人民币官方价按汇率 7.2 折算）：DeepSeek 有峰谷（北京时间 9:00–12:00、14:00–18:00 高峰=空闲×2）；Kimi（K2/K3）、GLM（4.x/5.x）恒价。未收录的模型不计费。",
