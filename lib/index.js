...
function emptyUsageStore() {
  ...
  return { version: 12, days: {}, months: {}, total: {}, steps: {}, cursors: {} };
}
...
function foldDiskSession(usageStore, liveModels, liveProviders, sessionId, file) {
  ...
  for (const event of events) {
    if (foldUsageEvent(usageStore, liveModels, liveProviders, session, event)) folded++;
  }
  return folded;
}
...
function foldUsageEvent(store, liveModels, liveProviders, session, event) {
  ...
  // request/header 先于 usage 事件到达，携带本次请求的真实模型与来源；
  // 会话的 requestContext() 只反映"当前"值，跨模型/跨来源切换的会话用它会归错桶。
  if (event.type === "request/header") {
    ...
    if (typeof provider === "string" && provider) liveProviders.set(session.id, provider);
    return true;
  }
  ...
  // 来源分组：opencode-go 订阅 → go；其余（DeepSeek 官方直连等）→ api
  // 来源判定：显式 opencode-go → go；显式其他 → api；
  // 无 provider 信息时，仅限 OpenCode Go 独占模型归 go（其余保守归 api）
  const rawProvider = liveProviders.get(session.id);
  let groupKey;
  if (rawProvider === "opencode-go") {
    groupKey = "go|";
  } else if (rawProvider) {
    groupKey = "api|";
  } else if (model === "ox-alpha-free") {
    groupKey = "go|"; // 仅在 OpenCode Go 目录中提供
  } else {
    groupKey = "api|";
  }
  ...
  addDelta(store.days[day] || (store.days[day] = {}), modelKey, d);
  addDelta(store.months[month] || (store.months[month] = {}), modelKey, d);
  addDelta(store.total, modelKey, d);
  return true;
}
...
function handleTokens(usageStore, req, res) {
  ...
  const collect = (bucket, field) => {
    for (const [bucketKey, entry] of Object.entries(bucket || {})) {
      if (!entry || typeof entry !== "object") continue;
      // 桶键格式：go|模型 或 api|模型
      const sep = bucketKey.indexOf("|");
      const group = sep > 0 ? bucketKey.slice(0, sep) : "api";
      const modelName = sep > 0 ? bucketKey.slice(sep + 1) : bucketKey;
      let record = models.get(bucketKey);
      if (!record) {
        record = { key: bucketKey, group, model: modelName, total: zero(), month: zero(), week: zero(), today: zero() };
        models.set(bucketKey, record);
      }
  ...
  // 本月逐日（月初 → 今天，缺数据的日子零填充），供月视图图表使用
  const monthStart = new Date(now);
  monthStart.setDate(1);
  monthStart.setHours(0, 0, 0, 0);
  const monthDays = [];
  for (let t = monthStart.getTime(); t <= now; t += 86400000) {
    const key = dayKey(t);
    monthDays.push({ date: key, ...sumBucket(usageStore.days[key]) });
  }
  sendJson(res, 200, {
  ...
  function replaySession(session) {
    try {
      for (const event of session.events || []) {
        if (foldUsageEvent(usageStore, liveModels, liveProviders, session, event)) schedulePersist();
      }
    } catch (error) {
    ...
    if (rebuilt) {
      ...
      try {
        ...
        for (const [sessionId, file] of logs) {
          ...
          if (foldDiskSession(usageStore, liveModels, liveProviders, sessionId, file) > 0) foldedSessions++;
        }
        if (foldedSessions > 0) {
          schedulePersist();
          // eslint-disable-next-line no-console
