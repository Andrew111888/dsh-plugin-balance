...
function emptyUsageStore() {
  // v2: per-model buckets keep cache-hit reads apart from cache-miss input,
  // and accumulate the estimated cost in the peak / idle billing bands.
  return { version: 2, days: {}, months: {}, total: {}, steps: {}, cursors: {} };
}
// v1 只记录了 {in, out}（in 含缓存命中），没有费用与时段信息。
...
function migrateLegacyStore(store) {
  ...
  // 只折算 days / months / total 三个按模型分桶的聚合。
  const buckets = [store.days, store.months, store.total];
  for (const bucket of buckets) {
    for (const [model, entry] of Object.entries(bucket || {})) {
      if (!entry || typeof entry !== "object") continue;
      if (entry.costPeak !== undefined && entry.costOff !== undefined) continue;
  ...
  try {
    ...
    if (store.version < 2) {
      ...
      if (migrated > 0) {
        ...
        console.log(`dsh-plugin-balance: 已将 ${migrated} 条 v1 用量记录折算为峰谷费用估算`);
      }
    }
    return store;
  } catch {
