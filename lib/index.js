...
function emptyUsageStore() {
  // v5: per-model buckets keep cache-hit reads apart from cache-miss input,
  // and accumulate the estimated cost in the peak / idle billing bands.
  return { version: 5, days: {}, months: {}, total: {}, steps: {}, cursors: {} };
}
// v1/v2 早期数据只有 {in,out}（in 含缓存命中），没有费用与时段信息。
// 一次性折算：cacheRead 字段按命中单价、in 按未命中单价（v1 的 in 里
// 夹带的缓存命中无法拆分，按未命中价计、轻微高估）；时段按每日高峰
// 6h（9:00–12:00、14:00–18:00）占 1/4 的比例摊到峰/谷两档。
// 对"曾被误写"的时间桶，直接整体重估（SET），不叠加旧值。
function migrateLegacyEntry(entry, model) {
  const pricing = pricingForModel(model);
  const miss = Math.max(0, Number(entry.in) || 0);
  const hit = Math.max(0, Number(entry.cacheRead) || 0);
  ...
  entry.costPeak = (miss * pricing.peak.miss + hit * pricing.peak.hit + out * pricing.peak.out) / 1e6 / 4;
  entry.costOff = (miss * pricing.idle.miss + hit * pricing.idle.hit + out * pricing.idle.out) / 1e6 * 3 / 4;
}
function repairUsageStore(store) {
  let repaired = 0;
  // 老数据没有费用/时段信息，统一按 1/4 高峰占比折算（SET 覆盖、幂等）；
  // total 是 { model: entry } 平铺，days/months 是 { 日期: { model: entry } }；
  // steps 以 stepKey 为键、不知道模型，且会被新样本整体替换，无需折算。
