...
/* 展开（macOS 风）：从锚点生长 + 模糊渐清 + 淡入，弹性缓动 */
"@keyframes dsh-balance-pop{0%{opacity:0;transform:scale(.82) translateY(-6px);filter:blur(6px)}55%{opacity:1;filter:blur(1.5px)}100%{opacity:1;transform:scale(1) translateY(0)}}",
/* 收起：加速缩小 + 轻微模糊渐出 */
"@keyframes dsh-balance-out{0%{opacity:1;transform:scale(1)}100%{opacity:0;transform:scale(.97) translateY(-4px);filter:blur(3px)}}",
/* 面板展开：下滑淡入 + 模糊渐清 */
"@keyframes dsh-balance-panel{from{opacity:0;transform:translateY(-8px);filter:blur(2px)}to{opacity:1;transform:translateY(0)}}",
/* 下拉菜单：顶部弹出 */
...
/* 主行错峰淡入（展开时略晚于卡片外壳） */
"@keyframes dsh-balance-stagger{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}",
/* Apple 风格缓动：慢起 → 快速滑入 → 轻收尾 */
".dsh-balance-pop{transform-origin:0 0;animation:dsh-balance-pop .32s cubic-bezier(.22,1,.36,1) both}",
".dsh-balance-out{transform-origin:0 0;animation:dsh-balance-out .2s cubic-bezier(.4,0,1,1) both}",
".dsh-balance-panel{animation:dsh-balance-panel .26s cubic-bezier(.22,1,.36,1) both;animation-delay:.07s}",
".dsh-balance-menu{transform-origin:top center;animation:dsh-balance-menu .16s cubic-bezier(.22,1,.36,1) both}",
...
".dsh-balance-stagger{animation:dsh-balance-stagger .3s cubic-bezier(.22,1,.36,1) both;animation-delay:.06s}",
...
"@media (prefers-reduced-motion: reduce){.dsh-balance-spin,.dsh-balance-pop,.dsh-balance-out,.dsh-balance-panel,.dsh-balance-menu,.dsh-balance-num,.dsh-balance-pulse,.dsh-balance-fade,.dsh-balance-stagger{animation:none!important}.dsh-balance-btn,.dsh-balance-pill,.dsh-balance-bar{transition:none!important}}",
].join("\n");
...
/* 收起时先播放退出动画（200ms，与 dsh-balance-out 时长一致），再切换成胶囊；减少动态偏好下立即收起。 */
