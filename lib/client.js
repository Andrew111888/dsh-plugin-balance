...
/* 周期徽章宽度过渡（悬停展开/收起 5h→三周期） */
".dsh-balance-chip{transition:max-width .26s cubic-bezier(.19,.9,.28,1),opacity .22s ease-out}",
...
"@media (prefers-reduced-motion: reduce){.dsh-balance-spin,.dsh-balance-pop,.dsh-balance-pill-in,.dsh-balance-out,.dsh-balance-panel,.dsh-balance-panel-out,.dsh-balance-menu,.dsh-balance-menu-item,.dsh-balance-num,.dsh-balance-pulse,.dsh-balance-fade,.dsh-balance-stagger{animation:none!important}.dsh-balance-btn,.dsh-balance-pill,.dsh-balance-bar,.dsh-balance-chip{transition:none!important}}",
].join("\n");
...
/* 用量型主行：默认只显示 5h（rolling）徽章，悬停 0.2s 展开全部周期，移开恢复。 */
...
var isDefault = period.id === defaultPeriodId;
var visible = usageHover || isDefault;
return h("span", {
key: period.id,
...
opacity: visible ? 1 : 0,
transitionDelay: visible && !isDefault ? (index * 30) + "ms" : "0ms",
},
},
h("span", {
style: {
...
? h("span", { key: "pill-multi", className: "dsh-balance-num", style: { display: "inline-flex", alignItems: "center", fontVariantNumeric: "tabular-nums" } },
...
key: p.id,
className: "dsh-balance-chip",
style: { display: "inline-flex", overflow: "hidden", maxWidth: visible ? 80 : 0, opacity: visible ? 1 : 0 },
},
...
onPointerUp: endDrag,
onPointerCancel: endDrag,
onMouseEnter: isUsage ? widgetHoverEnter : undefined,
onMouseLeave: isUsage ? widgetHoverLeave : undefined,
},
mainRow,
