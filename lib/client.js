...
/* 面板展开：下滑淡入 + 模糊渐清 + 弹簧收尾 */
"@keyframes dsh-balance-panel{0%{opacity:0;transform:translateY(-12px) scale(.985);filter:blur(4px)}55%{opacity:1}80%{transform:translateY(1.5px) scale(1.004)}100%{opacity:1;transform:translateY(0) scale(1)}}",
...
/* 下拉菜单：顶部弹出 + 轻微过冲 */
"@keyframes dsh-balance-menu{0%{opacity:0;transform:scale(.92) translateY(-6px)}60%{opacity:1}82%{transform:scale(1.02) translateY(0)}100%{opacity:1;transform:scale(1) translateY(0)}}",
...
".dsh-balance-panel{animation:dsh-balance-panel .42s cubic-bezier(.19,.9,.28,1) both}",
".dsh-balance-panel-out{animation:dsh-balance-panel-out .12s cubic-bezier(.4,0,.8,1) both}",
".dsh-balance-menu{transform-origin:top center;animation:dsh-balance-menu .26s cubic-bezier(.19,.9,.28,1) both}",
...
"@media (prefers-reduced-motion: reduce){.dsh-balance-spin,.dsh-balance-pop,.dsh-balance-pill-in,.dsh-balance-out,.dsh-balance-panel,.dsh-balance-panel-out,.dsh-balance-menu,.dsh-balance-menu-item,.dsh-balance-num,.dsh-balance-pulse,.dsh-balance-fade,.dsh-balance-stagger{animation:none!important}.dsh-balance-btn,.dsh-balance-pill,.dsh-balance-bar{transition:none!important}}",
].join("\n");
...
/* 面板高度弹簧：wrapper 高度从 from 连续过渡到 to（约 0.45s，k=110/c=18），
