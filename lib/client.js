...
card.style.opacity = String(1 - 0.1 * p);
} else {
card.style.transform = "translateY(" + (-8 * p) + "px) scale(" + (1 - 0.06 * p) + ")";
...
var chipCooldownUntilRef = React.useRef(0);
var USAGE_HOVER_EXPAND_MS = 200;
var USAGE_HOVER_FULL_MS = 3000;
var CHIP_HOVER_COOLDOWN_MS = 450;
...
clearUsageTimers();
// 冷却期：刚因离开收起过徽章，忽略本次进入（打断 收起→变大→离开→进入 的死循环）
if (performance.now() < chipCooldownUntilRef.current) return;
if (isUsage) {
if (reducedMotionRef.current) {
setUsageHover(true);
} else {
...
// 仅在指针其实还在胶囊内部时忽略（DOM 切换/尺寸变化导致的偶发误报）；
// 真离开（哪怕只出去 1px）都收起徽章。
if (pointerInsideElement(e, boxRef.current)) return;
clearUsageTimers();
chipCooldownUntilRef.current = performance.now() + CHIP_HOVER_COOLDOWN_MS;
setUsageHover(false);
}
/* 指针是否严格在元素矩形内（无外扩） */
...
/* 卸载时清理收起动画 / 胶囊悬停 / 面板切换 / 高度弹簧 / 徽章弹簧 / 自动收起 定时器 */
React.useEffect(function () {
return function () {
