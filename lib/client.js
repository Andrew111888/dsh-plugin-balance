...
var hoverExpandWatchRef = React.useRef(null);
var hoverExpandedRef = React.useRef(false);
...
// 立刻卸载面板：消除详情页高度带来的"高盒子残影"，卡片只剩主行
setState(function (prev) {
return Object.assign({}, prev, { panel: null });
});
// 等面板卸载（下一帧）后再测量短卡片，FLIP 精确缩到胶囊矩形（无中间形态）
collapseRafRef.current = requestAnimationFrame(function () {
var card = boxRef.current;
var fromRect = card ? card.getBoundingClientRect() : null;
var toRect = pillRectRef.current;
var canFlip = !!(fromRect && toRect && fromRect.width > 0 && toRect.width > 0);
var k = 140, c = 28, m = 1;
...
if (canFlip) {
var ox = toRect.left - fromRect.left;
var oy = toRect.top - fromRect.top;
var sx = toRect.width / fromRect.width;
var sy = toRect.height / fromRect.height;
card.style.transform = "translate(" + (ox * p) + "px," + (oy * p) + "px) scale(" + (1 + (sx - 1) * p) + "," + (1 + (sy - 1) * p) + ")";
card.style.opacity = String(1 - 0.12 * p);
} else {
card.style.transform = "translateY(" + (-6 * p) + "px) scale(" + (1 - 0.14 * p) + ")";
...
/* 移开自动缩回：展开后 600ms 宽限期（避免点击展开瞬间的小幅移动误触发），
 * 之后离开卡片 350ms 才收起；期间移回即取消。 */
function scheduleAutoCollapse() {
if (closingRef.current) return;
var sinceExpand = performance.now() - expandAtRef.current;
var delay = Math.max(AUTO_COLLAPSE_LEAVE_MS, AUTO_COLLAPSE_GRACE_MS - sinceExpand);
if (autoCollapseTimerRef.current) window.clearTimeout(autoCollapseTimerRef.current);
autoCollapseTimerRef.current = window.setTimeout(function () {
autoCollapseTimerRef.current = null;
// 保险：指针其实还在卡片内（DOM 切换偶发误报 mouseleave）时不收起
var root = boxRef.current;
if (root && typeof root.matches === "function" && root.matches(":hover")) return;
requestCollapse();
}, delay);
}
...
/* 悬停展开的卡片：胶囊与卡片可能不在同一位置（锚点翻转），mouseleave 不可靠，
 * 用全局 mousemove 监测——指针持续离开卡片 350ms 且距展开超 1s 才收起。 */
function armHoverExpandWatch() {
disarmHoverExpandWatch();
var outsideSince = 0;
function onMove(e) {
var card = boxRef.current;
...
if (inside) { outsideSince = 0; return; }
var now = performance.now();
if (!outsideSince) outsideSince = now;
if (now - outsideSince >= 350 && now - expandAtRef.current >= 1000) {
disarmHoverExpandWatch();
requestCollapse();
}
}
document.addEventListener("mousemove", onMove, true);
hoverExpandWatchRef.current = onMove;
}
function disarmHoverExpandWatch() {
if (hoverExpandWatchRef.current) {
document.removeEventListener("mousemove", hoverExpandWatchRef.current, true);
hoverExpandWatchRef.current = null;
}
}
...
// 悬停展开的卡片：不走 mouseleave（位置翻转时不可靠会误收），只走全局离开监测
...
/* 卸载时清理收起动画 / 胶囊悬停 / 面板切换 / 高度弹簧 / 徽章弹簧 / 自动收起 / 离开监测 */
React.useEffect(function () {
return function () {
...
cancelAutoCollapse();
disarmHoverExpandWatch();
// 手动点开：启用 mouseleave 自动缩回（悬停展开才走全局监测）
hoverExpandedRef.current = false;
}
setState(function (prev) {
...
if (hoverExpandedRef.current) return; // 悬停展开：只走全局离开监测
