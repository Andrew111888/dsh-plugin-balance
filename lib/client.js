...
/* 展开（macOS 风）：从锚点生长 + 模糊渐清 + 淡入，长尾缓动 + 弹簧收尾 */
"@keyframes dsh-balance-pop{0%{opacity:0;transform:scale(.7) translateY(-12px);filter:blur(10px)}55%{opacity:1;filter:blur(2px)}82%{transform:scale(1.015) translateY(0)}100%{opacity:1;transform:scale(1) translateY(0)}}",
/* 收起：加速缩小 + 轻微模糊渐出 */
"@keyframes dsh-balance-out{0%{opacity:1;transform:scale(1)}100%{opacity:0;transform:scale(.96) translateY(-4px);filter:blur(3px)}}",
/* 面板展开：下滑淡入 + 模糊渐清 */
"@keyframes dsh-balance-panel{from{opacity:0;transform:translateY(-10px);filter:blur(3px)}to{opacity:1;transform:translateY(0)}}",
/* 下拉菜单：顶部弹出 */
"@keyframes dsh-balance-menu{from{opacity:0;transform:scale(.96) translateY(-4px)}to{opacity:1;transform:scale(1) translateY(0)}}",
/* 数字/文案切换：轻微上滑淡入 */
...
".dsh-balance-pop{transform-origin:0 0;animation:dsh-balance-pop .52s cubic-bezier(.19,.9,.28,1) both}",
".dsh-balance-pill-in{transform-origin:0 0;animation:dsh-balance-pop .3s cubic-bezier(.19,.9,.28,1) both}",
".dsh-balance-out{transform-origin:0 0;animation:dsh-balance-out .26s cubic-bezier(.4,0,.8,1) both}",
".dsh-balance-panel{animation:dsh-balance-panel .3s cubic-bezier(.19,.9,.28,1) both;animation-delay:.18s}",
".dsh-balance-menu{transform-origin:top center;animation:dsh-balance-menu .2s cubic-bezier(.19,.9,.28,1) both}",
...
".dsh-balance-stagger{animation:dsh-balance-stagger .34s cubic-bezier(.19,.9,.28,1) both;animation-delay:.12s}",
...
"@media (prefers-reduced-motion: reduce){.dsh-balance-spin,.dsh-balance-pop,.dsh-balance-pill-in,.dsh-balance-out,.dsh-balance-panel,.dsh-balance-menu,.dsh-balance-num,.dsh-balance-pulse,.dsh-balance-fade,.dsh-balance-stagger{animation:none!important}.dsh-balance-btn,.dsh-balance-pill,.dsh-balance-bar{transition:none!important}}",
].join("\n");
...
/* 收起时先播放退出动画（260ms，与 dsh-balance-out 时长一致），再切换成胶囊；减少动态偏好下立即收起。 */
...
/* 折叠胶囊悬停 3 秒 → 展开并打开明细面板 */
var hoverTimerRef = React.useRef(null);
var PILL_HOVER_OPEN_MS = 3000;
...
function openFromPillHover() {
hoverTimerRef.current = null;
setState(function (prev) {
if (!prev.collapsed) return prev;
return Object.assign({}, prev, { collapsed: false, panel: "detail" });
});
}
function schedulePillOpen() {
clearPillHover();
hoverTimerRef.current = window.setTimeout(openFromPillHover, PILL_HOVER_OPEN_MS);
}
...
onMouseEnter: schedulePillOpen,
onMouseLeave: clearPillHover,
title: "悬停 3 秒打开明细，单击展开，可拖动调整位置",
style: {
...
? h("span", { key: "pill-multi", className: "dsh-balance-num", style: { display: "inline-flex", alignItems: "center", gap: 6, fontVariantNumeric: "tabular-nums" } },
periods.map(function (p) {
var pct = Math.round(p.percent);
return h("span", { key: p.id, style: { fontWeight: 700, color: usageColor(pct) } }, pct + "%");
})
)
...
panelNode = periodRows.length > 0 ? h("div", { key: "detail-usage", className: "dsh-balance-panel", style: { padding: "2px 0 6px" } }, periodRows)
: h("div", { key: "detail-empty", className: "dsh-balance-panel", style: { padding: "12px 16px", color: T.text3, fontSize: 12, lineHeight: "18px" } }, "套餐接口未返回可显示的用量。");
} else {
...
h("span", { style: { fontSize: 11, lineHeight: "15px", color: T.text3, fontWeight: 600, letterSpacing: ".04em" } }, label),
h("div", { style: { display: "flex", alignItems: "baseline", gap: 6, flexWrap: "wrap", minWidth: 0 } },
h("span", { key: "tk-" + label + "-" + bigValue, className: "dsh-balance-num", style: { fontSize: 18, lineHeight: "24px", fontWeight: 800, color: accentColor || T.text, fontVariantNumeric: "tabular-nums" } }, bigValue),
costBadge(agg)
),
...
var label = isToday ? "今天" : String(day.date || "").slice(5);
return h("div", {
key: day.date,
...
h("span", { style: { fontSize: 9, lineHeight: "12px", color: isToday ? T.accent : T.text3, fontWeight: isToday ? 700 : 400, whiteSpace: "nowrap" } }, label)
);
})
),
...
h("div", { style: { padding: "6px 12px 4px", fontSize: 11, lineHeight: "15px", fontWeight: 600, color: T.text3, letterSpacing: ".06em", borderTop: "1px solid color-mix(in srgb, " + T.border + " 70%, transparent)" } }, "按模型统计"),
modelRows
));
} else {
