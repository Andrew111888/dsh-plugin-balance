...
// 分组视图：按宿主记录的来源过滤（go=OpenCode Go 订阅，api=DeepSeek 官方直连）
var wantGroup = modelGroup;
for (var gi = 0; gi < periodModels.length; gi++) {
var ge = periodModels[gi];
if (ge.m.group !== wantGroup) continue;
groupRows.push(modelRowEl(ge.m, ge.a, groupRows.length));
}
} else {
// 全部：同名模型跨来源合并显示
var mergedByName = new Map();
for (var mi2 = 0; mi2 < periodModels.length; mi2++) {
var me = periodModels[mi2];
...
var allList = [...mergedByName.values()];
allList.sort(function (a, b) { return b.a.tokens - a.a.tokens; });
for (var ai2 = 0; ai2 < allList.length; ai2++) {
groupRows.push(modelRowEl(allList[ai2].m, allList[ai2].a, ai2));
}
}
