...
/* 显示名：全部视图下给同名不同源的模型加来源后缀 */
function modelDisplayName(m) {
if (!m.group) return m.model;
return m.model + (m.group === "go" ? " · Go" : " · 官方");
}
...
/* Catmull-Rom → cubic Bézier 平滑曲线 */
var line = "M" + pts[0].x.toFixed(2) + "," + pts[0].y.toFixed(2);
for (var si = 0; si < n - 1; si++) {
var p0 = pts[Math.max(0, si - 1)];
var p1 = pts[si];
var p2 = pts[si + 1];
var p3 = pts[Math.min(n - 1, si + 2)];
var c1x = p1.x + (p2.x - p0.x) / 6, c1y = p1.y + (p2.y - p0.y) / 6;
var c2x = p2.x - (p3.x - p1.x) / 6, c2y = p2.y - (p3.y - p1.y) / 6;
line += " C" + c1x.toFixed(2) + "," + c1y.toFixed(2) + " " + c2x.toFixed(2) + "," + c2y.toFixed(2) + " " + p2.x.toFixed(2) + "," + p2.y.toFixed(2);
}
var lastPt = pts[n - 1];
return h("svg", { viewBox: "0 0 " + W + " " + H, preserveAspectRatio: "none", style: { width: "100%", height: 52, display: "block" } },
h("defs", null,
h("linearGradient", { id: "dsh-balance-area-fade", x1: "0", y1: "0", x2: "0", y2: "1" },
h("stop", { offset: "0%", stopColor: T.accent, stopOpacity: 0.22 }),
h("stop", { offset: "70%", stopColor: T.accent, stopOpacity: 0.06 }),
h("stop", { offset: "100%", stopColor: T.accent, stopOpacity: 0 })
)
),
h("line", { x1: 0, y1: H - 0.5, x2: W, y2: H - 0.5, stroke: T.border, strokeWidth: 0.5, vectorEffect: "non-scaling-stroke" }),
h("path", { d: line + " L" + W + "," + H + " L0," + H + " Z", fill: "url(#dsh-balance-area-fade)", stroke: "none" }),
h("path", { d: line, fill: "none", stroke: T.accent, strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", vectorEffect: "non-scaling-stroke" }),
h("circle", { cx: lastPt.x, cy: lastPt.y, r: 3.5, fill: T.accent, opacity: 0.18, vectorEffect: "non-scaling-stroke" }),
h("circle", { cx: lastPt.x, cy: lastPt.y, r: 1.8, fill: T.accent, vectorEffect: "non-scaling-stroke" })
);
}
...
// 全部：每个模型×来源单独一条，不合并（同名模型官方/Go 分开显示）
sectionEntries = periodModels.slice();
}
...
h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 } },
h("span", { style: { fontSize: 12, lineHeight: "18px", fontWeight: 650, color: T.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, modelDisplayName(m)),
