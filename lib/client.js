...
/* 分组：全部 / DeepSeek API（官方直连）/ OpenCode Go */
var modelGroupDefs = [["all", "全部"], ["deepseek", "DeepSeek API"], ["opencode", "OpenCode Go"]];
function isDeepSeekModel(name) {
return String(name || "").toLowerCase().indexOf("deepseek") !== -1;
}
var periodModels = [];
...
if (modelGroup === "deepseek" || modelGroup === "opencode") {
// 分组视图
var wantDeepSeek = modelGroup === "deepseek";
var sumAgg = tokenZeroAgg;
var groupMembers = [];
for (var gi = 0; gi < periodModels.length; gi++) {
var ge = periodModels[gi];
var gIsDs = isDeepSeekModel(ge.m.model);
if (gIsDs !== wantDeepSeek) continue;
if (wantDeepSeek) {
sumAgg = {
in: sumAgg.in + (Number(ge.a.in) || 0),
cacheRead: sumAgg.cacheRead + (Number(ge.a.cacheRead) || 0),
out: sumAgg.out + (Number(ge.a.out) || 0),
tokens: sumAgg.tokens + (Number(ge.a.tokens) || 0),
costPeak: sumAgg.costPeak + (Number(ge.a.costPeak) || 0),
costOff: sumAgg.costOff + (Number(ge.a.costOff) || 0),
cost: sumAgg.cost + (Number(ge.a.cost) || 0),
};
} else {
groupMembers.push(ge);
}
}
if (wantDeepSeek) {
if (sumAgg.tokens > 0) {
groupRows.push(modelRowEl({ model: "DeepSeek API（官方直连）" }, sumAgg, 0));
}
} else {
for (var oi = 0; oi < groupMembers.length; oi++) {
groupRows.push(modelRowEl(groupMembers[oi].m, groupMembers[oi].a, oi));
}
}
} else {
// 全部
for (var ai = 0; ai < periodModels.length; ai++) {
groupRows.push(modelRowEl(periodModels[ai].m, periodModels[ai].a, ai));
}
}
...
miniToggle(modelGroupDefs, modelGroup, setModelGroup)
),
groupRows.length > 0 ? groupRows : h("div", { style: { padding: "6px 12px 8px", color: T.text3, fontSize: 12, lineHeight: "18px" } }, "该分组在此周期暂无用量。")
));
