...
".dsh-balance-root button:disabled{opacity:.45;cursor:default}",
"@media (prefers-reduced-motion: reduce){.dsh-balance-spin{animation:none!important}}",
].join("\n");
...
actions.push(iconButton("close", "折叠为小悬浮球", closeIcon(), toggleCollapsed, {
...
isUsage ? usageChips : h("div", { style: amountStyle }, amountText),
statusRow
),
...
h("div", { style: { height: "100%", width: percent + "%", borderRadius: 3, background: color, transition: "width .3s ease" } })
),
period.resetsAt ? h("div", { style: Object.assign({}, smallTextStyle(T.text3), { marginTop: 5 }) }, resetText(period.resetsAt) + " 重置") : null
);
});
panelNode = periodRows.length > 0 ? h("div", { style: { borderTop: "1px solid " + T.border } }, periodRows)
: h("div", { style: { padding: "8px 12px", borderTop: "1px solid " + T.border, color: T.text3, fontSize: 12, lineHeight: "18px" } }, "套餐接口未返回可显示的用量。");
} else {
...
panelNode = rows.length > 0 ? h("div", { style: { borderTop: "1px solid " + T.border } }, rows)
: h("div", { style: { padding: "8px 12px", borderTop: "1px solid " + T.border, color: T.text3, fontSize: 12, lineHeight: "18px" } }, "余额接口未返回可显示的余额条目。");
}
...
h("span", { style: { fontSize: 15, lineHeight: "20px", fontWeight: 750, color: accentColor || T.text, fontVariantNumeric: "tabular-nums" } }, formatTokens(agg ? agg.tokens : null)),
h("span", { style: { fontSize: 10, lineHeight: "14px", color: T.text3, whiteSpace: "nowrap" } },
agg ? ("入 " + formatTokens(agg.in) + " · 出 " + formatTokens(agg.out)) : "—"
)
);
};
...
return h("div", {
key: m.model || index,
title: "今日 " + formatTokens(m.today && m.today.tokens) + " · 本月 " + formatTokens(m.month && m.month.tokens),
style: {
display: "flex",
flexDirection: "column",
gap: 2,
...
h("span", { style: { fontSize: 13, lineHeight: "18px", fontWeight: 700, color: T.accent, fontVariantNumeric: "tabular-nums", flex: "none" } }, formatTokens(m.total && m.total.tokens))
),
h("span", { style: smallTextStyle(T.text3) }, "入 " + formatTokens(m.total && m.total.in) + " · 出 " + formatTokens(m.total && m.total.out))
);
});
...
panelNode = h("div", { style: { borderTop: "1px solid " + T.border, paddingBottom: 4 } }, tokenPanelChildren);
...
panelNode = h("div", { style: { borderTop: "1px solid " + T.border, padding: "10px 12px 12px", display: "flex", flexDirection: "column", gap: 9 } },
h("div", null,
